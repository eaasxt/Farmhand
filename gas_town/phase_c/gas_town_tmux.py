...
class GasTownTmux:
    ...
    def setup_tmux_config(self) -> bool:
        ...
        try:
            ...
            print(f"✅ Gas Town tmux config written to {tmux_config_file}")
            ...
            print(f"✅ Status bar script created at {status_script_file}")

            # Instructions for user
            print(f"\n📋 To enable Gas Town tmux integration:")
...
set -g status-right "#[fg=yellow]⚡#({status_script_path}) #[fg=green]%H:%M #[fg=blue]%Y-%m-%d"
...
set -g status-left "#[fg=colour166,bold]🏭 Gas Town #[fg=colour244]| "
...
python3 "{script_path}" status-bar-data 2>/dev/null || echo "🏭 Gas Town"
'''
        return script
    ...
    def get_status_bar_data(self) -> str:
        ...
        try:
            ...
            status_parts = []
            status_parts.append(f"🪝{len(all_hooks)}")

            if total_pending > 0:
                if total_pending > 10:
                    status_parts.append(f"📋{total_pending}!")
            ...
            if agent_sessions:
                status_parts.append(f"👥{len(agent_sessions)}")
            ...
            return " ".join(status_parts) if status_parts else "🏭 Gas Town"

        except Exception as e:
            return f"🏭 Gas Town ({str(e)[:10]})"
    ...
    def spawn_agent_session(self, role: str = "Agent", session_name: Optional[str] = None) -> bool:
        ...
        try:
            ...
            print(f"🚀 Spawning Gas Town {role} session: {session_name}")
            ...
            cmd = [
                ...
                f"echo '🏭 Gas Town {role} Agent: {agent_name}' && "
            ...
            if result.returncode == 0:
                ...
                try:
                    ...
                    print(f"🪝 Hook created for {agent_name}: {hook_id}")
                except Exception as e:
    ...
    def list_agent_sessions(self) -> None:
        ...
        if not sessions:
            ...
            print("\n💡 Create new agent session with:")
            print("   ./gt tmux spawn <role>")
            return

        for session in sessions:
            status_icon = "🟢" if session.get('attached') else "🔴"
    ...
    def integrate_gt_commands(self) -> None:
        ...
        print("🔗 GAS TOWN TMUX INTEGRATION")
        print("=" * 50)
        ...
        print()
        print("Status bar shows: 🪝hooks 📋pending 👥agents ⚡GUPP")
    ...
    def attach_to_agent(self, session_name: str) -> bool:
        ...
        try:
            ...
            if not matching_sessions:
                print(f"❌ No agent session found matching: {session_name}")
            ...
            print(f"🔗 Attaching to session: {target_session}")
    ...
    def kill_agent_session(self, session_name: str) -> bool:
        ...
        try:
            ...
            target_session = matching_sessions[0]['name']
            print(f"🗑️ Killing session: {target_session}")
            ...
            if result.returncode == 0:
                print(f"✅ Session {target_session} terminated")
                return True
            else:
                print(f"❌ Failed to kill session {target_session}")
                return False

        except Exception as e:
            print(f"❌ Session kill failed: {e}")
            return False
...
def main():
    ...
    if args.command == "setup":
        ...
        if not args.target:
            print("❌ Session name required for attach")
            return 1
        ...
        if not args.target:
            print("❌ Session name required for kill")
            return 1
