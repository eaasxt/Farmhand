...
class TestMEOWStackValidation(unittest.TestCase):
    ...
    def test_beads_integration(self):
        ...
        self.assertEqual(molecule.state, MoleculeState.CREATED)

        # Test state transitions (bead-like lifecycle)
        self.molecule_state.start_molecule(mol_id)
        status = self.molecule_state.get_molecule_status(mol_id)
    ...
    def test_molecule_workflows(self):
        ...
        self.molecule_state.start_molecule(workflow_mol)
        ...
        status = self.molecule_state.get_molecule_status(workflow_mol)
    ...
    def test_crash_recovery_ndi(self):
        ...
        self.molecule_state.start_molecule(crash_mol)
        ...
        recovered_status = crashed_state.get_molecule_status(crash_mol)
        self.assertEqual(recovered_status["state"], "running")

        # Get last checkpoint to resume from
        history = crashed_state.get_molecule_history(crash_mol)
        last_checkpoint = history[-1] if history else None
        self.assertIsNotNone(last_checkpoint)

        # Complete from recovered state
        crashed_state.complete_molecule(crash_mol, {"recovered": True, "final_state": "success"})

        final_status = crashed_state.get_molecule_status(crash_mol)
    ...
    def test_molecule_templates_protomolecules(self):
        ...
        self.assertEqual(len(history), len(instantiated_config["steps"]))
    ...
    def test_swarm_coordination_meow_integration(self):
        ...
        print(f"   - Swarm team formed: {len(team.members)} agents")
